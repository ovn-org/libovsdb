...
var defDB, _ = model.NewDatabaseModelRequest("Open_vSwitch",
...
func BenchmarkUpdate1(b *testing.B) {
	...
	require.NoError(b, err)
	dbModelReq, err := model.NewDatabaseModelRequest("Open_vSwitch", map[string]model.Model{
		"Bridge":       &Bridge{},
		"Open_vSwitch": &OpenvSwitch{},
	})
	require.NoError(b, err)
	ovs.primaryDB().cache, err = cache.NewTableCache(&s, dbModelReq, nil)
	require.NoError(b, err)
	update := []byte(`{
		"Open_vSwitch": {

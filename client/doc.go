...
Based on these Models a Database Model (see DatabaseModelRequest type) is built to represent
the entire OVSDB:
...
The DatabaseModelRequest represents the entire Database (or the part of it we're interested in).
...
A client instance is created by simply specifying the connection information and the database model:
     ...
     ovs, _ := client.Connect(context.Background(), dbModelReq)

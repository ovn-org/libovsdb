...
func WithTable(m model.Model, fields ...interface{}) MonitorOption {
	return func(o *ovsdbClient, monitor *Monitor) error {
		tableName := o.primaryDB().model.Request().FindTable(reflect.TypeOf(m))
		if tableName == "" {
			return fmt.Errorf("object of type %s is not part of the DatabaseModelRequest", reflect.TypeOf(m))
		}
